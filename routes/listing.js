	...
	Listing.findById(id, (err, listing) => {
		if (err) {
			...
			for(let i = 0; i < listing.imageNumber; i++){
				listing.imageLinks.push(process.env.NODE_ENV === 'production'
					? `http://images.mynuho.me/${id}/${i}`
					: `http://staging.my-nu-home-1513614055126.appspot.com.storage.googleapis.com/${id}/${i}`);
			}
